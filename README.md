...
> 지원하는 한국 채용 플랫폼에서 채용 공고를 수집, 추출, 저장하고 정규화된 형태로 제공하는 FastAPI 백엔드와 React 프런트엔드로 구성된 모노레포입니다.
...
CareerOS는 FastAPI 백엔드(`career_os_api`)와 React 프런트엔드(`career_os_web`)로 구성된 모노레포입니다. 현재 구현은 `saramin.co.kr`와 `wanted.co.kr` 채용 공고 수집, OpenAI 기반 구조화 추출, Google 로그인 기반 사용자별 저장에 초점을 두고 있습니다.

백엔드는 다음 HTTP 엔드포인트를 제공합니다.
...
- `main.py`는 FastAPI 앱을 조합하고, 앱 lifespan 동안 PostgreSQL connection pool을 만들고, DDL을 적용하며, `CORSMiddleware`와 `SessionMiddleware`를 연결하고 `/v1` 라우터를 마운트합니다.
...
- `GET /v1/auth/google`는 선택적 `callback_url` 쿼리 파라미터를 지원합니다. 제공되면 콜백 시 `GoogleLoginResponse` JSON 대신 `{callback_url}?access_token={token}`으로 리다이렉트합니다. 프런트엔드가 이 방식으로 OAuth 토큰을 수신합니다.
- 보호된 라우트는 `get_current_user()`를 통해 인증을 검증합니다. 이 의존성은 세션 쿠키(`user_id`)를 먼저 확인하고, 없으면 Bearer 토큰(JWT)으로 대체합니다. 두 경로 모두 활성 사용자만 허용합니다.
- 사용자 정보는 `PATCH /v1/auth/me`로 수정할 수 있으며, `update_user_name()`으로 이름을 갱신합니다.
- 채용 공고 영속화는 `(user_id, platform, posting_id)` 기준 `INSERT ... ON CONFLICT DO UPDATE`를 사용하므로, 같은 사용자가 같은 원본 공고를 다시 저장해도 기존 레코드를 갱신합니다.
- 목록 조회는 큰 텍스트 필드를 제외한 summary projection만 반환하고, 상세 조회는 전체 저장 레코드를 반환합니다. 기본 `limit`은 `20`입니다.
- 외부 fetch는 `detect_platform()`으로 지원 도메인을 먼저 검증합니다. 따라서 추출 대상은 현재 지원하는 플랫폼으로 제한됩니다.
- `validate_posting_id()`는 posting ID가 숫자로만 구성되었는지 검증하여, 잘못된 ID가 서로 다른 공고를 같은 키로 충돌시키는 것을 방지합니다.
- Saramin 추출은 내부 AJAX 엔드포인트와 iframe 인라인 전략을 사용하고, Wanted 추출은 내부 REST API 응답을 HTML로 재구성해 같은 추출 파이프라인으로 넘깁니다.
- `main.py`에 `CORSMiddleware`가 `SessionMiddleware` 앞에 추가되어 있으며, `allowed_origins` 설정값으로 프런트엔드 도메인을 허용합니다.

### 런타임 진입점

- `main.py`
  FastAPI 앱 객체 `career_os`를 만들고, lifespan hook에서 PostgreSQL pool과 스키마를 초기화한 뒤 `CORSMiddleware`, `SessionMiddleware`, `v1_router`를 연결합니다.
...
graph TD
    ...
    Fetch --> Platform["platform.py<br/>도메인 allowlist + posting id 검증"]
...
1. 브라우저 또는 프론트엔드 클라이언트가 `GET /v1/auth/google`을 호출하면 Authlib가 Google 로그인 페이지로 리다이렉트합니다. 선택적 `callback_url` 파라미터를 전달하면 세션에 저장됩니다.
2. `GET /v1/auth/google/callback`은 Google 토큰을 교환하고 userinfo를 읽어 `users` 테이블에 upsert한 뒤, 내부 사용자 UUID를 `sub`로 담은 JWT access token을 발급합니다. `callback_url`이 세션에 있으면 `{callback_url}?access_token={token}`으로 리다이렉트하고, 없으면 `GoogleLoginResponse` JSON을 반환합니다.
3. 클라이언트가 해당 Bearer access token과 함께 보호된 엔드포인트를 호출하면 `get_current_user()`가 세션 쿠키를 먼저 확인하고, 없으면 JWT를 검증하여 현재 사용자를 조회합니다.
...
| 검증 및 설정    | Pydantic, `pydantic-settings`                                                 | 요청/응답 모델과 환경 변수 기반 설정 로딩                     |
...
│   ├── router.py
│   └── schemas.py
├── career_os_web/                 ← React 프런트엔드 (별도 README 참조)
├── .python-version
├── main.py
├── pyproject.toml
├── tests/
...
- `career_os_api/constants.py`는 BS4 파서 식별자, OpenAI 추출 시스템 프롬프트, 플랫폼별 User-Agent 문자열, API 버전 접두사(`"v1"`)를 정의합니다.
...
- `career_os_web/`는 React 프런트엔드 SPA입니다. 상세한 구조와 설정은 `career_os_web/README.md`를 참조하세요.
...
- `callback_url` 쿼리 파라미터를 통한 프런트엔드 리다이렉트 지원
- 세션 쿠키와 Bearer 토큰 이중 인증 (세션 우선)
- Bearer 토큰으로 현재 사용자 조회 및 보호된 라우트 접근 제어
- 사용자 이름 수정 (PATCH /auth/me)
- 로그아웃 및 세션 종료 (POST /auth/logout)
- `saramin.co.kr`, `wanted.co.kr` 지원 플랫폼 판별
- 플랫폼별 URL 형식에서 posting ID 추출과 숫자형 검증 (`validate_posting_id()`)
...
- 사용자별 summary row 기반 offset pagination 목록 API (기본 limit=20)
- 사용자 범위로 제한된 상세 조회 API
- CORS 미들웨어로 프런트엔드 도메인 허용
...
설정은 `pydantic-settings`를 통해 로컬 환경 변수 파일 또는 프로세스 환경 변수에서 로드됩니다. 로컬 전용 설정은 커밋하지 않는 파일에 `KEY=value` 형식으로 작성해 두는 방식을 권장합니다.
...
| `ALLOWED_ORIGINS`      | 아니오 | 프로덕션 프런트엔드 주소와 로컬 개발 오리진 목록             | CORS 허용 오리진 목록 (쉼표 구분)                       |
| `SECRET_KEY`           | 예     | 없음                                                         | 세션 서명과 JWT 서명에 사용하는 애플리케이션 비밀 키    |
| `JWT_ALGORITHM`        | 아니오 | `HS256`                                                      | JWT 서명 알고리즘                                       |
| `JWT_EXPIRE_MINUTES`   | 아니오 | `10080`                                                      | access token 만료 시간(분), 기본값은 7일                |
...
테스트는 `tests/conftest.py`에서 `DATABASE_URL`, `OPENAI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `SECRET_KEY`를 테스트용 값으로 덮어쓰므로, 현재 테스트 스위트를 실행할 때 별도 로컬 환경 변수 파일은 필요하지 않습니다.
...
필수 로컬 설정은 커밋하지 않는 환경 변수 파일에 아래 형식으로 작성합니다.
...
선택 설정을 추가할 때도 같은 형식으로 `ALLOWED_ORIGINS`, `JWT_ALGORITHM`, `JWT_EXPIRE_MINUTES`, `HTTP_FETCH_TIMEOUT`, `HTTP_IMAGE_TIMEOUT`, `OPENAI_MODEL`, `OPENAI_TEMPERATURE`, `MAX_IMAGES`를 이어서 작성하면 됩니다.

로컬에서 Google 로그인을 테스트하려면 `REDIRECT_URI`를 `<local-api-origin>/v1/auth/google/callback` 형태로 맞추고, 같은 값을 Google Cloud Console에도 등록해야 합니다. 값을 생략하면 기본값은 FastAPI Cloud 배포 URL을 사용합니다.
...
- `<api-base-url>/v1/docs`
- `<api-base-url>/v1/redoc`
...
| `GET`   | `/v1/auth/google`             | Google 로그인 시작                   | 브라우저를 Google 로그인 페이지로 리다이렉트. 선택적 `callback_url` 쿼리 파라미터 지원                                                                    |
| `GET`   | `/v1/auth/google/callback`    | Google 로그인 콜백 처리              | Google userinfo를 저장/갱신하고 `GoogleLoginResponse`를 반환. `callback_url`이 있으면 `{callback_url}?access_token={token}`으로 리다이렉트. 실패 시 `400` |
| `GET`   | `/v1/auth/me`                 | 현재 사용자 조회                     | Bearer 토큰 필요, 세션 쿠키 또는 JWT를 검증하고 `users` 테이블의 활성 사용자 정보를 반환                                                                  |
...
| `GET`   | `/v1/job-postings`            | 페이지네이션된 요약 목록             | Bearer 토큰 필요, 현재 사용자가 저장한 공고만 반환, 쿼리 파라미터: `offset >= 0`, `1 <= limit <= 100`, 기본값 `limit=20`                                  |
...
curl \
  ...
  '<api-base-url>/v1/job-postings/extraction?url=https://www.saramin.co.kr/zf_user/jobs/relay/view?rec_idx=4930'
```
...
- `tests/unit/auth`
  JWT 생성/검증과 `get_current_user()` 의존성의 세션/Bearer 이중 인증, 사용자 복원, 잘못된 토큰 처리에 대한 단위 테스트입니다.
- `tests/unit/service/job_posting`
  플랫폼 판별, posting ID 검증, fetch 오케스트레이션, Saramin/Wanted 처리, 스키마 검증, 추출 메시지 구성, 이미지 수집, refusal 처리에 대한 단위 테스트입니다.
- `tests/unit/database`
...
| 프런트엔드   | Vercel                          | `career_os_web/vercel.json`에 SPA rewrite 설정 |
...
| 이중 인증                  | `career_os_api/auth/dependencies.py`                                                                                         | 세션 쿠키와 Bearer 토큰을 모두 지원해 브라우저와 API 클라이언트 호환성 확보 |
| CORS 경계 설정             | `main.py`, `career_os_api/config.py`                                                                                         | 프런트엔드 도메인만 허용해 교차 출처 요청을 제어                            |
...
- Google 로그인 후 콜백이 실패하는 경우
  로컬에서는 `REDIRECT_URI`를 `<local-api-origin>/v1/auth/google/callback` 형태로 맞추고, 같은 값을 Google Cloud Console에 등록해야 합니다.
...
- 추출 시 `400 Bad Request`
  현재는 `saramin.co.kr`와 `wanted.co.kr` 호스트만 허용됩니다. Saramin URL에는 `rec_idx`가 필요하고, Wanted URL은 `/wd/{id}` 형식을 따라야 합니다. posting ID는 숫자로만 구성되어야 합니다.
...
- CORS 오류
  프런트엔드 도메인이 `ALLOWED_ORIGINS`에 포함되어야 합니다. 값은 쉼표로 구분하고, 사용하는 배포 프런트엔드 주소와 로컬 개발 오리진을 함께 넣어야 합니다.
...
현재 기여 시 참고할 수 있는 문서로는 `tests/README.md`와 저장소 내부 작업 규칙 문서가 있습니다. 이를 기준으로 보면 기대되는 로컬 작업 흐름은 다음과 같습니다.
