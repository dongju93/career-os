...
- `career_os_api/service/job_posting/`은 URL 분류, 원격 HTML 조회, Saramin/Wanted 전용 콘텐츠 수집, OpenAI 기반 구조화를 담당합니다.
...
- Saramin 추출은 전용 fetch 경로를 사용하며, 플랫폼의 AJAX 상세 엔드포인트를 호출하고 iframe 콘텐츠를 인라인한 뒤 OpenAI 추출 단계로 넘깁니다. Wanted 추출은 `/api/v4/jobs/{id}` 응답을 추출용 HTML로 재구성한 뒤 같은 추출 단계로 넘깁니다.
...
| HTML 파싱       | Beautiful Soup 4                         | 상위 HTML 파싱, Saramin 전용 섹션 추출, Wanted HTML 재구성 처리 |
...
- `career_os_api/service/job_posting/`은 핵심 수집 파이프라인입니다. 플랫폼 판별, fetch 로직, Saramin/Wanted 전용 처리, 추출 프롬프트 구성, Pydantic 스키마를 포함합니다.
...
| `HTTP_FETCH_TIMEOUT` | 아니오 | `30.0`         | 채용 공고 페이지 fetch 및 Saramin/Wanted 요청 타임아웃(초) |
...
- `tests/unit/service/job_posting`
  플랫폼 판별, fetch 오케스트레이션, Saramin/Wanted 처리, 추출 메시지 구성, 이미지 수집, refusal 처리에 대한 단위 테스트입니다.
- `tests/unit/database`
