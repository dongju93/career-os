...
CREATE INDEX IF NOT EXISTS idx_chatkit_threads_user_updated ON chatkit_threads (user_id, updated_at DESC, id);
...
CREATE TABLE IF NOT EXISTS chatkit_items (
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW (),
    UNIQUE (id, user_id),
    FOREIGN KEY (thread_id, user_id) REFERENCES chatkit_threads (id, user_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_chatkit_items_thread_created ON chatkit_items (thread_id, user_id, created_at, id);