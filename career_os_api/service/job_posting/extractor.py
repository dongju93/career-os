...
def _build_messages(
    ...
    """
    # Field map: domain cues and special normalizations only.
    # Global rules (null-if-absent, no invent, full-copy long fields, images) live
    # once in EXTRACTION_SYSTEM_PROMPT — avoid restating them per field (gpt-5.6-luna).
    extraction_prompt = (
        "Extract all job posting fields from the content below.\n\n"
        "=== IDENTITY (echo exactly) ===\n"
        f"platform: {platform.value}\n"
        f"posting_url: {source_url}\n"
        f"posting_id: {posting_id}\n\n"
        "=== FIELD MAP (source cues / special rules) ===\n"
        "company_name: company or organization name. KO: 회사명, 기업명\n"
        "job_title: exact posting title. Content is pre-processed: h1 is "
        "'[채용공고 제목] <title text>' — use the text after that marker. "
        "If the marker is missing, use the most prominent role name at the top. "
        "Never use the 채용직군 category value (e.g. '개발·데이터') as the title.\n"
        "experience_req: as written (e.g. '3년 이상', '신입', '경력 무관', 'Entry level')\n"
        "education_req: as written (e.g. '학력 무관', '대졸 이상', \"Bachelor's required\")\n"
        "employment_type: as written (e.g. '정규직', '계약직', '인턴', 'Full-time', 'Contract')\n"
        "location: work location. KO: 근무지, 근무위치\n"
        "deadline: calendar date → YYYY-MM-DD; descriptive text keep as written "
        "(e.g. '채용 시 마감', 'Until filled')\n"
        "salary: as written (e.g. '4,000만원 이상', '회사 내규에 따름', 'Negotiable')\n"
        "job_description: company/role intro. KO: 회사 소개, 직무 소개\n"
        "responsibilities: work to do. KO: 담당업무, 주요업무, 업무 내용\n"
        "qualifications: required quals. KO: 자격요건, 지원 자격, 필수 요건\n"
        "preferred_points: nice-to-haves. KO: 우대사항, 우대 조건\n"
        "benefits: perks/welfare. KO: 혜택 및 복지, 복리후생\n"
        "hiring_process: steps in order. KO: 전형 절차, 채용 프로세스\n"
        "tech_stack: technologies for this role as a list of strings "
        "['Python', 'Django', 'PostgreSQL']. Include languages, frameworks, databases, "
        "cloud, tools. Exclude generic process terms (e.g. 'Agile', 'OKR').\n"
        "tags: hashtags exactly as in source. KO: 태그 (e.g. ['#스타트업', '#원격근무'])\n"
        "application_method: how to apply (e.g. '홈페이지 접수', 'Email application')\n"
        "application_form: application URL or form name\n"
        "contact_person: recruiter/contact name\n"
        "homepage: company website URL\n"
        "job_category: function as written (e.g. '백엔드 개발', 'Software Engineer')\n"
        "industry: domain as written (e.g. 'IT/인터넷', 'Fintech')\n\n"
        f"--- CONTENT ---\n{text_content}\n--- END CONTENT ---"
    )
